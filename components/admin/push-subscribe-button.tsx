...
export function PushSubscribeButton({ vapidKey }: { vapidKey: string }) {
  ...
  if (state === "needs-install") {
    return (
      <div className="bg-amber-600/10 border border-amber-500/20 rounded-xl px-4 py-3.5 space-y-2">
        <div className="flex items-center gap-2">
          <Smartphone size={15} className="text-amber-400 flex-shrink-0" />
          <p className="text-xs font-semibold text-amber-300">Installa l&apos;app per le notifiche</p>
        </div>
  ...
  return (
    <button
      ...
      className="flex items-center gap-2 w-full bg-amber-600/10 border border-amber-500/20 hover:border-amber-500/40 rounded-xl px-4 py-3 transition-colors disabled:opacity-50"
    >
      {state === "requesting"
      ...
      <div className="text-left">
        <p className="text-xs font-semibold text-amber-300">Abilita notifiche</p>
        <p className="text-xs text-neutral-500">Un avviso ad ogni vendita, con il nome dell&apos;affiliata</p>
      </div>
    </button>
  );
}
